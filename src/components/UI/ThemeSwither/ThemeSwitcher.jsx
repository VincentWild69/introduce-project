...
import { THEME_LIGHT, THEME_DARK } from './../../../context/ThemeProvider';
...
const ThemeSwitcher = () => {
  ...
  const onThemeChange = (e) => {
    e.target.checked ? theme.change(THEME_DARK) : theme.change(THEME_LIGHT);
  }

  return (
    <div className={s.themeSwitcherContainer}>
      <label className={s.themeSwitcher}>
        <input onChange={onThemeChange} type='checkbox' className={s.themeCheckbox} checked={theme.theme === THEME_DARK}/>
        <span className={s.themeSlider}>
          <span className={s.themeSliderState}>
